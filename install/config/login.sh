...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the gnk file
    echo "splash" | sudo tee -a /etc/cmdline.d/gnk.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the gnk file
    echo "quiet" | sudo tee -a /etc/cmdline.d/gnk.conf
  fi
...
if [ "$(plymouth-set-default-theme)" != "gnk" ]; then
  sudo cp -r "$HOME/.local/share/gnk/default/plymouth" /usr/share/plymouth/themes/gnk/
  sudo plymouth-set-default-theme -R gnk
fi
...
if [ ! -f /etc/systemd/system/gnk-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/gnk-seamless-login.service
[Unit]
Description=GNK Seamless Auto-Login
Documentation=https://github.com/basecamp/gnk
...
# Enable gnk-seamless-login.service only if not already enabled
if ! systemctl is-enabled gnk-seamless-login.service | grep -q enabled; then
  sudo systemctl enable gnk-seamless-login.service
fi
